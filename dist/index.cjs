...
class ImpersonatedWallet {
	...
	async setNonce(x) {
		await this.provider.send('anvil_setNonce', [this.address, ethers.ethers.toBeHex(x)]);
	}
...
class Foundry extends FoundryBase {
	...
	async deploy(arg0) {
		...
		let tx, c;
		if (salt) {
			salt = ethers.ethers.toBeHex(salt, 32);
			tx = await from.sendTransaction({
				to: create2Deployer,
				data: ethers.ethers.concat([salt, unsigned.data]),
			});
			c = new ethers.ethers.Contract(ethers.ethers.getCreate2Address(create2Deployer, salt, ethers.ethers.keccak256(unsigned.data)), abi, from);
			c.__create2 = {deployer: create2Deployer, salt};
		} else {
			tx = await from.sendTransaction(unsigned);
			c = new ethers.ethers.Contract(ethers.ethers.getCreateAddress(tx), abi, from);
		}
		let receipt = await tx.wait(confirms);
...
function result_to_object(result, inputs, deep = true) {
	if (Array.isArray(inputs)) {
		...
		inputs.forEach((ty, i) => {
			...
			let v = result[i];
			if (deep && ty.isTuple() && v instanceof ethers.ethers.Result) {
