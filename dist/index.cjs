...
async function compile(sol, options = {}) {
	...
	await promises.writeFile(config_file, encode({profile: {[profile]: config}}));
...
class FoundryBase extends EventEmitter {
	...
	async exportArtifacts(dir, {force = true, tests = false, scripts = false} = {}) {
		let args = [
			...
			// if you pass: "--out=/"
			'--out', node_path.join(this.root, dir),
		];
	...
	async find({file, contract}) {
		...
		contract ??= node_path.basename(file); // derive contract name from file name
	...
	async fileArtifact(arg0) {
		...
		if (typeof file === 'object') { // inline artifact (this might be dumb)
			...
			json = JSON.parse(await promises.readFile(file));
			type = 'artifact';
		} else { // source file
...
class FoundryDeployer extends FoundryBase {
	...
	static async mainnet(pkey, rest = {}) {
		...
		const provider = new ethers.ethers.JsonRpcProvider('https://rpc.ankr.com/eth', CHAIN_MAINNET, {staticNetwork: true});
		const wallet = new ethers.ethers.Wallet(pkey, provider);
		return this.load({...rest, wallet})
	}
	static async sepolia(pkey, rest = {}) {
		if (!(pkey instanceof ethers.ethers.SigningKey)) pkey = new ethers.ethers.SigningKey(pkey);
		const provider = new ethers.ethers.JsonRpcProvider('https://rpc.ankr.com/eth_sepolia', CHAIN_SEPOLIA, {staticNetwork: true});
		const wallet = new ethers.ethers.Wallet(pkey, provider);
		return this.load({...rest, wallet});
	}
	static async load({
	...
	async prepare(arg0) {
		...
		const ret = {
			...
			...fees,
			wei,
			eth: ethers.ethers.formatEther(wei),
			root,
			cid,
			linked,
			compiler,
			...
			async json() {
				...
				if (type === 'code') {
					...
					json.sources = Object.fromEntries(Object.entries(json.sources).map(([k, v]) => {
						return [k.startsWith('../') ? node_path.normalize(node_path.join(this.root, k)) : k, v];
					}));
				}
				return json;
			},
	...
	async verifyEtherscan({json, cid, address, apiKey, encodedArgs, compiler, pollMs = 5000, retry = 3} = {}) {
		...
		if (encodedArgs) {
			encodedArgs = ethers.ethers.hexlify(encodedArgs);
...
class Foundry extends FoundryBase {
	...
	async deploy(arg0) {
		...
		let receipt = new ethers.ethers.ContractTransactionReceipt(abi, this.provider, await tx.wait(confirms));
	...
	findEvent(event) {
		if (event instanceof ethers.ethers.EventFragment) { // solo fragment
			try {
				return this.findEvent(event.topicHash);
			} catch (err) {
				return {
					abi: new ethers.ethers.Interface([event]),
					frag: event
				};
			}
		}		
		if (event.includes('(')) { // signature => topicHash
			event = ethers.ethers.EventFragment.from(event).topicHash;
		}
...
exports.FoundryDeployer = FoundryDeployer;
exports.Node = Node;
